...
input double lots_user = 0.012 ;
void OnTick(){
  // Getting the bid price
  ...
  // Getting the low price - The lowest candle in tbe 
  int low_index = iLowest(_Symbol, period, MODE_LOW, count, 0) ; 
  double low = iLow(_Symbol, period, low_index) ; 
  ...
  // Removing excess precision(lots) - rounding up
  double lots = lots_user ;  
  ...
  // Condition to check whether sell or buy - nested ternary operator
 
  bool isBuy =  (ask > high) ; 
  bool isSell = (bid < low) ; 
  // Checking for open positions
  bool isOpen = loopPositions(isBuy, isSell) ; 
  ...
  if (ask > high && !isOpen){
     ...
     double tp = NormalizeDouble((ask + ((tp_points+spread+stop_level) * _Point)), _Digits) ;
     ...
     if (Trade.Buy(lots, _Symbol, 0, sl, tp, "Buy position")){
  ...
  // Opening a selling position - downward trend
  if(low > bid){
      // Calculate the stop loss and take profit
      double sl = NormalizeDouble((bid + ((stop_level+spread+sl_points)*_Point)), _Digits) ; 
      double tp = NormalizeDouble((bid - ((stop_level+spread+sl_points)*_Point)), _Digits) ;
      // Validate sl and tp
      if (tp > 0 && (ask - tp) < (stop_level + spread) * _Point){
          Print("Invalid take profit value") ; 
          return ; 
      } 
      if (sl > 0 && (sl - ask) < (spread + stop_level) * _Point){
          Print("Invalid stop loss value") ;
          return ;  
      }
      // Opening a sell position
      if ( Trade.Sell(lots, _Symbol, 0, sl, tp, "Sell position")){
         Print("Trade Successful: ", Trade.ResultOrder()) ;  
         if (PositionSelect(_Symbol)){
            double profit = PositionGetDouble(POSITION_PROFIT) ; 
            Print("Current Profit: ", profit) ; 
...
bool loopPositions(bool isBuy, bool isSell){
   ...
   for (int i = PositionsTotal()-1 ; i>=0 ; i--){
       ...
       // Modfication for a sell position - dynamically moving the take profit to minimize loss
       else if(lsTsl && isSell){
          // Get the position take profit and stop loss
          double position_sl = PositionGetDouble(POSITION_SL) ; 
          double position_tp = PositionGetDouble(POSITION_TP) ;
          double sl_buffer = tslBufferPips * _Point ; 
          double last_candle_tp = NormalizeDouble((iHigh(_Symbol, period, 1) + sl_buffer), _Digits) ;
           if (last_candle_tp < position_tp && last_candle_tp > 0) {
               // Modify the position with new trailing TP
               Trade.PositionModify(position_ticket, position_sl, last_candle_tp);
               Print("Trailing TP Updated: ", last_candle_tp);
          }     
       }
